...
class JsonSerializer(BaseSerializer):
    ...
    INT_PATTERN = fr"[+-]?\d+" #+ ili - i potom rasnoe kol-vo cifr
    FLOAT_PATTERN = fr"(?:[+-]?\d+(?:\.\d+)?(?:e[+-]?\d+)?|[+-]?{INF_LITERAL}\b|{NAN_LITERAL}\b)" #ecsponencialnaia chast chisla shobi uchest
    ...
    def __loads_to_dict(self, string: str):
        ...
        if string[0] == '{' and string[-1] == '}':
            ...
            return {self.__loads_to_dict(matches[i][0]):
              ...
              #u nas buset spisok podriat key znachenie,key znachenie i t.d. perebiraem cherez 1 v kachestve key I v kachestve znachenie i+1
        else:
            raise ValueError
