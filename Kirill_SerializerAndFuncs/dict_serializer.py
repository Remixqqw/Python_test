...
from Kirill_SerializerAndFuncs import nonetype, moduletype, codetype, celltype, \
                          ...
                          CODE_PROPS, UNIQUE_TYPES, gentype

class DictSerializer: #serealizuet slovar, kluchevie slova
    TYPE_KW = "type"
    SOURCE_KW = "source"
    ...
    def to_dict(cls, obj, is_inner_func=False):
        ...
        if type(obj) is dict:
            ...
            return {cls.TYPE_KW: dict.__name__, #type idet kak slovar
                    cls.SOURCE_KW: [[cls.to_dict(item[0]), cls.to_dict(item[1])] for item in obj.items()]} # a sourse kak para kluch znachenie

        if type(obj) in (set, frozenset, tuple, bytes, bytearray, gentype):
            return {cls.TYPE_KW: type(obj).__name__,
                    cls.SOURCE_KW: cls.to_dict([*obj])} #slovar sozdannii is etoi kollekcii

        if type(obj) is complex:
            return {cls.TYPE_KW: complex.__name__,  #v tip pomeschaem nazvanie tipa
                    cls.SOURCE_KW: {complex.real.__name__: obj.real, #v kachestve source u nas slovar gde real part i serealizovano real
                                    complex.imag.__name__: obj.imag}} #i imag mnimaia chast budet zapisana mnimaia chast
        ...
        if isinstance(obj, property):
            source = {"fget": cls.to_dict(obj.fget),
                      "fset": cls.to_dict(obj.fset),
                      "fdel": cls.to_dict(obj.fdel)}
            return {cls.TYPE_KW: "property",
    ...
    @classmethod
    def __get_gvars(cls, func, is_inner_func): # is_inner_func eto ona vlojena ili ne vlojena
        name = func.__name__
        gvars = {}
        ...
        for gvar_name in func.__code__.co_names:
            ...
            if gvar_name in func.__globals__:
                ...
                # Class
                    ...
                    c = func.__globals__[gvar_name] #esli eto class  mi zapisivaem znachenie peremennoi v c
                    if is_inner_func and name in c.__dict__ and func == c.__dict__[name].__func__: #esli eto vlojennaia func i name etoi func soderjitsa v slovare etogo klassa i po etomu imeni lejit eta func
                        gvars[gvar_name] = c.__name__ #to mi po etoi func zapisivaem nazvanie
                    else:
                        gvars[gvar_name] = c
    ...
    def __get_obj_dict(cls, obj):
        ...
        for key, value in dct.items():
            if type(value) not in UNIQUE_TYPES: #UNIQUE_TYPES ne nujno serelizovat
    ...
    def from_dict(cls, obj, is_dict=False):
        ...
        if type(obj) not in (dict, list):
            ...
            if obj_type == gentype.__name__:
                l = obj_source
                return (i for i in l)

            if obj_type == complex.__name__:
                return obj_source[complex.real.__name__] + \
                    obj_source[complex.imag.__name__] * 1j #1j- mnimaia edenica

            if obj_type == moduletype.__name__:
                return __import__(obj_source)

            if obj_type == codetype.__name__:
                return codetype(*[cls.from_dict(obj_source[prop]) for prop in CODE_PROPS]) #peredaem v konstruktor type razimenovanii spisok, v slovare lejat properti,i izvlekaem po ley nujnie znachenia

            if obj_type == celltype.__name__:
                return celltype(cls.from_dict(obj_source))

            if obj_type == "property":
                return property(fget=cls.from_dict(obj_source["fget"]),
                                fset=cls.from_dict(obj_source["fset"]),
                                fdel=cls.from_dict(obj_source["fdel"]))
            ...
            if obj_type == functype.__name__: #esli eto functype to mi vosossdaem is slovaria
                ...
                func = functype(code, gvars, name, defaults, closure) #kak mi eti 5 razlojili tak i vosstanavlivaem

                # Restoring recursion
                if func.__name__ in gvars: #shtobi ne bilo rekursii
            ...
            if obj_type == type.__name__:
                ...
                dct = {cls.from_dict(item[0]): cls.from_dict(item[1]) for item in dct.items()} #kajdii ego element is formata preobrazuem

                cl = type(name, bases, dct) #sozdaem klass is etix 3 shtuk

                # Restore a reference to the current class in the nested method __globals__
                for attr in cl.__dict__.values(): # perebiraem atributi v dict
